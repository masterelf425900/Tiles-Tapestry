...
class CollageApp(tk.Tk):
    ...
    def open_image_dialog(self, path, orig_w, orig_h, entry=None):
        ...
        lock_check.grid(row=4, column=0, columnspan=2, pady=2)
        ttk.Label(frame, text="(Leave manual positions blank for auto–layout)").grid(row=5, column=0, columnspan=2, pady=2)
        ...
        def on_ok():
            ...
            # When an image is locked, do not reposition it in auto-layout.
            self.recalc_layout()
            self.refresh_listbox()
            dialog.destroy()
    ...
    def show_preview(self):
        ...
        ttk.Button(top_bar, text="Zoom In", style="Accent.TButton", command=self.zoom_in).pack(side=tk.LEFT, padx=5)
        ttk.Button(top_bar, text="Zoom Out", style="Accent.TButton", command=self.zoom_out).pack(side=tk.LEFT, padx=5)
        ...
        # Details panel with Info, Lock toggle, and manual X/Y editing
        self.details_panel = ttk.Frame(main_frame, padding=10)
        self.details_panel.pack(side=tk.LEFT, fill=tk.Y)
        self.details_label = ttk.Label(self.details_panel, text="Image Info")
    ...
    def update_preview(self):
        ...
        if cw > max_w:
            ...
            base_scale = 1
        final_scale = base_scale * self.preview_zoom_factor
        preview_img = collage_img.resize((int(cw * final_scale), int(ch * final_scale)), Image.LANCZOS)
        self.zoom_label.config(text=f"{int(final_scale*100)}%")
        self.preview_photo = ImageTk.PhotoImage(preview_img)
        self.canvas.delete("all")
        canvas_width = self.canvas.winfo_width() or preview_img.width
        canvas_height = self.canvas.winfo_height() or preview_img.height
        self.canvas.create_image(canvas_width/2, canvas_height/2, anchor="center", image=self.preview_photo, tags="preview_image")
        self.preview_items.clear()
        # Calculate offset to center preview image
        offset_x = canvas_width/2 - preview_img.width/2
        offset_y = canvas_height/2 - preview_img.height/2
        for idx, entry in enumerate(self.images):
            ...
            rect = self.canvas.create_rectangle(x_s + offset_x, y_s + offset_y, x_s + w_s + offset_x, y_s + h_s + offset_y,
                                                outline="", width=2,
                                                tags=("img", f"img_{idx}"))
    ...
    def fit_to_window(self):
        # Adjust zoom factor so that the entire collage fits into the canvas.
        collage_img = self.build_collage()
        if collage_img is None:
            return
        cw, ch = collage_img.size
        ...
        if canvas_width == 0 or canvas_height == 0:
            return
        scale_x = canvas_width / cw
        scale_y = canvas_height / ch
        self.preview_zoom_factor = min(scale_x, scale_y)
        self.update_preview()
